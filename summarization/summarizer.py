...
def _build_corpus(sentences):
    ...
    #print (dictionary)
    #print ([dictionary.doc2bow(token) for token in split_tokens])
...
def summarize_corpus(corpus, sentences, ratio=0.2):
    """
...
def summarize(text, ratio=0.2, word_count=None, split=False):
    ...
    most_important_docs = summarize_corpus(corpus, sentences, ratio=ratio if word_count is None else 1)

    #######################################################################################
    #hashable_corpus = _build_hasheable_corpus(corpus)
    #sentences_by_corpus = dict(zip(hashable_corpus, sentences))
    #op = [sentences_by_corpus[tuple(important_doc)] for important_doc in corpus]
    #for i in op:
    #    print (i[1])
    #return [sentences_by_corpus[tuple(important_doc)] for important_doc in important_docs]
    #######################################################################################
